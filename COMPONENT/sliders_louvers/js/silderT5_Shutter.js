$(function() {
    ...
    function move(num) {
        ...
        $appendDiv.find('img').attr('src',$bigimgLi.eq(imgNow).find('img').attr('src')); 

        $appendDiv.css({zIndex:15}).find('div').css({'width':'10%'}).animate({'width':0},animateTime,function(){
           $appendDiv.css({zIndex:0}); 
        });
   
        tt = setTimeout(function(){
            $bigimgLi.eq(imgNext).animate({left:"0%"},10,function(){
                imgNow = imgNext;
                imgNext = null;
