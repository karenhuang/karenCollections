$(function() {
    ...
    if (!Array.prototype.shuffle) {
        Array.prototype.shuffle = function() {
            for(var j, x, i = this.length; i; j = parseInt(Math.random() * i), x = this[--i], this[i] = this[j], 
            this[j] = x);
            return this;
        };
    }

    //creat 50 divs than put background image
    ...
    function move(num) {
        ...
        $dd.css({'opacity':1});
        //push 0~49 in the array
        for(var i=0; i<totleSlice; i++){  arrayDiv.push(i); }
        //shuffle this array     
        arrayDiv.shuffle();
        ...
        function imgAnimate(eqIndex){
            totleTimes=eqIndex;
            if(totleTimes>0){  
                $dd.eq(arrayDiv[totleTimes]).animate({'opacity':0});     
