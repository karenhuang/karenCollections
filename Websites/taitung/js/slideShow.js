...
var SlideShow_2 = function(element) {
    ...
    target.nextId = 0;
    target.run = false;
    target.max = target.element.find(".pt-slide li").length - 1;
    target.drag = {
        startX: 0,
        endX: 0,
        prev: null,
    ...
    target.element.find(".pt-slide li").each(function(index, element) {
        $(element).css({
            left: index * 100 + "%"
        })
    });
        // 複製一份 ul.picList當小圖
    $(".pt-slide").clone().insertAfter($(".pt-slide")).addClass("list").removeClass("pt-slide");
    $(".list li").eq(target.now).addClass("on").siblings().removeClass("on");
    ...
    target.init = function(loop, animateTime) {
        ...
        target.move();
        if(target.max > 0){
            target.element.find(".pt-slide li").click(function() {
            ...
            target.element.find(".list li").click(function(){
                if (target.run) return;
                if (target.now != null || $(this).hasClass("on"))
                target.nextId = $(this).index()
                $(".list li").eq(target.now).addClass("on").siblings().removeClass("on");
                target.move();
                return false;
            ...
            target.element.find(".pt-slide").on("mousedown touchstart", function(e) {
                if (target.run) return;
            ...
            $(document).on("mousemove touchmove", function(e) {
                ...
                if (target.drag.state) {
                    ...
                    var tbanner = target.element.find(".pt-slide li");
    ...
    target.dragEnd = function(e) {
        ...
        if (target.drag.state && target.drag.move) {
            ...
            if (e.type == "mouseleave" || e.type == "touchleave" || new Date() - target.drag.date > 100) {
                ...
                target.element.find(".pt-slide a").click(function() {
                    return false;
                });
            } else {
                target.element.find(".pt-slide a").off();
            }
            target.move(true);
        }else{
            target.element.find(".pt-slide a").off();
    ...
    target.move = function(drag) {
        ...
        $(".list li").eq(target.nextId).addClass("on").siblings().removeClass("on");
        ...
        if (drag && target.now == target.nextId) {
            ...
            var nowElement = target.element.find(".pt-slide li").eq(target.now)
            var nextElement = target.element.find(".pt-slide li").eq(target.nextId);
            var nextNow;
