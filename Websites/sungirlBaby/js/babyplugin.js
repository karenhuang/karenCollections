...
$.fn.twoSliderShow = function(){
  ...
  function rightMove(num) {
    clearInterval(timer);
    if(rightPromise){
      leftUl.addClass('cover');
      rightUl.removeClass('cover');
      leftLi.find('.text').removeClass('textOp');
      rightLi.find('.text p').removeClass('pShow');  
      ...
      .stop().animate({left: "0%"},animateV, function(){
        ...
        rightLi.find('.text p').addClass('pShow');
  ...
  function leftMove(num){
    clearInterval(timer);
    if(leftPromise){
      rightUl.addClass('cover');
      leftUl.removeClass('cover');
      rightLi.find('.text').removeClass('textOp');
      leftLi.find('.text p').removeClass('pShow');       
      ...
      .stop().animate({left: "0%"},animateV, function() {
        ...
        leftLi.find('.text p').addClass('pShow');   
...
$.fn.fixedSliderShow = function(){
  ...
  function moveNext(num) {
    ...
    imgNext = imgNext > imgNow ? imgNext%imgMax : (imgNext+imgMax)%imgMax;  
    if(dragState){startPos = (startPos+dpos/bigimgLi.width()*100);}//重新計算拖動後的起始位置
    bigimgLi.eq(imgNow).stop().animate({top: nowAnimate},animateV);
    bigimgLi.eq(imgNext).css({"top": startPos+"%"})
